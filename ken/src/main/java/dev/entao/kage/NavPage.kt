...
fun HttpPage.boot(block: HtmlDoc.() -> Unit) {
	html {
		head.apply {
			metaCharset("UTF-8")
...
fun HttpPage.bootPage(block: Tag.() -> Unit) {
	boot {
		head.apply {
...
fun HttpPage.cardPage(block: Tag.() -> Unit) {
	sidebarPage {
		card {
			this.block()
		}
...
fun HttpPage.cardBodyPage(title: String, block: Tag.() -> Unit) {
	sidebarPage {
		card {
			cardHeader(title)
			cardBody {
