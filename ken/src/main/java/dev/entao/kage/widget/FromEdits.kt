...
fun Tag.labelTextAreaRow(p: Prop, editBlock: Tag.() -> Unit = {}) {
	formGroupRow {
		...
		val er = httpContext.httpParams.str(P.err(pname)) ?: ""
		val lb = this.label { +p.userLabel }
		this.div {
...
fun editConfig(editTag: Tag, p: Prop) {
	val pname = p.userName
	val er = editTag.httpContext.httpParams.str(P.err(pname)) ?: ""
	editTag.apply {
		if (er.isNotEmpty()) {
			addClass(B.isInValid)
		}
...
fun editConfig(editTag: Tag, editName: String) {
	val er = editTag.httpContext.httpParams.str(P.err(editName)) ?: ""
	editTag.apply {
		if (er.isNotEmpty()) {
			addClass(B.isInValid)
		}
