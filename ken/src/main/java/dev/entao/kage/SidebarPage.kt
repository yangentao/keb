...
import yet.web.api.FilesPage
import kotlin.reflect.KClass

fun HttpPage.sidebarPage(block: Tag.() -> Unit) {
...
fun HttpPage.checkAlertMessage(tag: Tag) {
	val er = httpParams.str(P.ERROR) ?: ""
	if (er.isNotEmpty()) {
		tag.alertError { +er }
	}
	val m = httpParams.str(P.SUCCESS) ?: ""
	if (m.isNotEmpty()) {
		tag.alertSuccess { +m }
	}
}

private fun HttpPage.buildUserInfoFlex(parentTag: Tag) {
